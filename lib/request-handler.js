...
exports.findMovieBuddies = function(req, response) {
  ...
  User.query(qb => {
      ...
      .then(allUsers => {
        ...
        var people = allUsersParsed.map(a => a.username);
        var Ids = allUsersParsed.map(a => a.id);
        console.log("THE FINALS!", people, Ids);
    ...
    //   console.log("THE CURRENT FINALS!", people, Ids);
 ...
 Rating.query(qb => {
        qb.select('rating.userid', 'rating.movieid','rating.score');
      })
      .fetchAll()
      .then(allRatings => {
          const allRatingsParsed = allRatings.models.map(models => models.attributes);
          console.log("this is an early respon  ", allRatingsParsed);
      });


      con.query('SELECT score,movieid,userid FROM ratings', (err, respon) => {
        console.log("This is respon  ", respon);
