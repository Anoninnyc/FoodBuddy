...
exports.getFriends = (req, res) => {
  ...
  var peopleId = [];
   ...
   // Rating.query( qb =>{
   //  qb.select('ratings.movieid');
   //  qb.where('ratings.userid', '=', userid);
   //  })
   //  .fetchAll()
   //  .then(res=>{
   //    console.log("this is resultss", res.models, "this is resultss");
   //  });

    Rating.forge({userid:userid}).fetchAll().then(results=>{

        console.log("this is 2resultss", results.models.map(res=> res.attributes).map(attrs=>[attrs.movieid, attrs.score]), "this is 2resultss");
    });

    // new Rating({userid:userid}).fetchAll().then(rez=>{
    //   console.log("this is 3resultss", rez.models, "this is 3resultss");
    // });
