...
exports.getFriends = (req, res) => {
  ...
  pool.getConnection((err, con) => {
    ...
    const innerJoinQuery='SELECT * from ratings INNER JOIN users WHERE ratings.userid = users.id AND username=?'

    con.query('SELECT score, movieid from ratings INNER JOIN users WHERE ratings.userid = users.id AND username=?', id, function(err, resp) {
        console.log("*******this is the innerjoin********", resp, "*******this is the innerjoin********");
        con.release();
    });
    ...
    con.query('SELECT id FROM users WHERE username = ?', id, function(err, resp) {
      ...
      console.log('this should be first person', userid);
