...
exports.sendRequest = (req, response) => {
  ...
  if (req.mySession.user === req.body.name) {
    ...
    pool.getConnection(function(err, con) {
      ...
      con.query('SELECT requestee, response FROM allrequests WHERE requestor = ? AND requestTyp =' + '"' + 'friend' + '"', req.mySession.user, (err, res) => {
        if (err) {
          throw err;
        }
        if (!res) {
...
exports.accept = function(req, response) {
  ...
  pool.getConnection((err, con) => {
    ...
    if (movie === '') {
      con.query('UPDATE allrequests SET response=' + '"' + 'yes' + '"' + '  WHERE requestor = ' + '"' + requestor + '"' + ' AND requestTyp=' + '"' + requestType + '"' + ' AND requestee = ' + '"' + requestee + '"', (err, res) => {
        if (err) throw err;
        console.log('Last insert ID:', res.insertId);
      })
    } else {
      con.query('UPDATE allrequests SET response=' + '"' + 'yes' + '"' + '  WHERE requestor = ' + '"' + requestor + '"' + ' AND movie=' + '"' + movie + '"' + ' AND requestee = ' + '"' + requestee + '"', (err, res) => {
        if (err) throw err;
        console.log('Last insert ID:', res.insertId);
      });
    }
...
exports.getThisFriendsMovies = (req, response) => {
  ...
  pool.getConnection((err, con) => {
    if (err) {
      console.log(err, 'getThisFriendsMovies');
      throw err;
    }
...
exports.findMovieBuddies = function(req, response) {
  ...
  pool.getConnection((err, con) => {
    if (err) {
      console.log(err, 'findMovieBuddies');
      throw err;
    }
...
exports.decline = function(req, response) {
  ...
  var movie = req.body.movie;
  var requestType = 'friend';
  var addOn = !movie ? ' AND requestTyp=' + '"' + requestType + '"' : ' AND movie =' + '"' + movie + '"';
  pool.getConnection((err, con) => {
    if (err) {
      console.log(err, 'decline');
      throw err;
    }
    con.query('UPDATE allrequests SET response=' + '"' + 'no' + '"' + ' WHERE requestor = ' + '"' + requestor + '"' + ' AND requestee=' + '"' + requestee + '"' + addOn, (err, res) => {
      if (err) {
        throw err;
      }
      console.log('Last insert ID:', res.insertId);
...
exports.getFriends = (req, res) => {
  ...
  var peopleId = [];
    ...
    Rating.forge({ userid, }).fetch().then(results => {
      console.log("RESULTSSSS",results);
        return results.models.map(res => res.attributes).map(attrs => [attrs.movieid, attrs.score])
    }).then(usersRatings => {
       console.log("these are usersRatings", usersRatings);
        pool.getConnection((err, con) => {
          if (err) {
            console.log(err, 'getFriends');
            throw err;
          }


    // con.query('SELECT id FROM users WHERE username = ?', id, (err, resp) => {
    //   //////
    //   // THIS QUERY is used so that no JOIN queries need be done.
    //   //////

    //   const userid = resp[0].id;


      // con.query('SELECT * FROM ratings WHERE userid = ?', userid, (err, resp) => {
      //   var usersRatings = resp.map(a=> ([a.movieid, a.score]));

      Relation.forge({ user1id: userid }).fetch().then(relations => {
        const rels = relations.models.map(res => res.attributes).map(attrs => attrs.user2id);
        console.log("this is rels", rels,userid);
        const peopleId2=[];
          for (let i = 0; i < rels.length; i++) {
            if (peopleId2.indexOf(rels[i]) === -1 && rels[i] !== userid) {
              peopleId2.push(rels[i]);
            }
          }
          console.log("this is the psuedo", peopleId2);
      });

        con.query('SELECT user2id FROM relations WHERE user1id = ?', userid, (err, resp) => {
          ...
          // console.log("this is the PEOPLEID", peopleId);
          const people = [];
          console.log('This should also be friends Ids', peopleId);
          const keyId = {};
          peopleId.forEach(a => {
            con.query('SELECT username FROM users WHERE id = ?', a, (err, resp) => {
              keyId[a] = resp[0].username;
              console.log('ONE of the people!!', resp[0].username);
              con.query('SELECT * FROM ratings WHERE userid =' + '"' + a + '"', (err, resp) => {
                if (!resp.length) {
                  resp = [{
                    userid: a,
